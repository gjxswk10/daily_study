...
>>>>>>> bb9e3e00108a21b4eb12f50b9ce599557c28f5b2
...
![error_flash_sort](.\photo\error_flash_sort.jpg)
=======
...
* 字向量层的激活函数。使用relu和sigmoid进行过测试，也试过不用激活函数；经验性的结论是，no activate > relu ~~ sigmoid。
* batch size的设置：之前一直以为batch size和模型的训练结果关系不大，所以我一直以来batch size都是设置成256；但测试发现，batch size = 32, batch size = 10的时候，结果确实会有很大不同。比如，同样的模型结构，batch size = 32时，模型F1值只有86.6%，改成10后，模型效果能够达到90%。按照我的理解，当batch size比较小时，相应的选择次数就会更多（指一个Epoch内得到不同参数的次数），模型会有更大的机会选择在验证集上表现更好的参数。
...
* 由于CCNN_WLSTM_CRF中，分向量和词向量的输入，我把二者分别处理成需要的dataset，但random时，顺序被打乱，输入的词向量并不对应输入的字向量，也就是说，在验证时，我给模型传入的完全是两个不相干的输入！！！
...
修改~/.bashrc这个不说了，之前就说过了。专门的位置在：/etc/sysconfig/i18n
